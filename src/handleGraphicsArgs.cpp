    ...
    rasterize(false), sampleRate(2)
{
  ...
  reg("rasterize", "scene object is being rendered with rasterizer", ArgumentParsing::NONE, 'z');
  reg("sampleRate", "sample rate of area light", ArgumentParsing::INT, 'x');
}
...
void GraphicsArgs::process(int argc, char *argv[])
{
  ...
  artistic = isSet("artistic");
  if (verbose) std::cout << "Artistic shading: ON" << std::endl;
  ...
  isSet("sampleRate", sampleRate);
  if (verbose) std::cout << "Setting sampleRate to " << sampleRate << std::endl;
